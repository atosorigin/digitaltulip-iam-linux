...
if (query != null){
	...
	// warren - we use id for uid
    if (query.get("left").equalsIgnoreCase("__UID__") && objectClass.equalsIgnoreCase("__ACCOUNT__")) query.put("left","id");
	// recon passes us in __NAME__ instead. Why?
    if (query.get("left").equalsIgnoreCase("__NAME__") && objectClass.equalsIgnoreCase("__ACCOUNT__")) query.put("left","id");
...
switch ( objectClass ) {
    case "__ACCOUNT__":
    sql.eachRow("SELECT * FROM customer" + where, {
	def email = [];
        sql.eachRow("SELECT * FROM customer_email" + where, { email.add( it.email ) });
	result.add([__UID__:it.id, __NAME__:it.id, uid:it.uid, userrole:it.userrole,firstname:it.firstname,lastname:it.lastname, email: email])
	});
...
log.info("return SEARCH result = " + result );    
return result;
