...
log.info("Entering "+action+" id=" + id +  " Script attrs=" + attributes) ;
...
switch ( objectClass ) {
    case "__ACCOUNT__":
    sql.execute("INSERT INTO customer (id,uid, firstname,lastname,userrole) values (?,?,?,?,?)",
        [
            id,
            attributes.get("uid").get(0),
            attributes.get("firstname").get(0),
            attributes.get("lastname").get(0),
            attributes.get("userrole").get(0),
        ]);
    // dont call commit it we are using autocommit
    //sql.commit();
    break
